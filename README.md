...
1. 웹의 `단지` 탭에서 수정할 단지의 `매물 수정`을 누르거나, 단지 상세에서 `이 단지 매물 수정`을 누릅니다.
2. 선택한 단지의 기존 JSON이 자동으로 열립니다. `listings` 배열을 최신 실제 매물로 수정합니다.
3. `JSON 검증 및 미리보기`로 평형별 가격 계산 결과를 확인합니다.
4. Cloudflare에 설정한 `ADMIN_SAVE_KEY` 값을 입력하고 `수정 내용 저장`을 누릅니다.
5. Cloudflare Pages 재배포가 완료되면 단지 상세와 대시보드에서 변경 데이터를 확인합니다.

신규 단지를 추가할 때:

1. 웹의 `JSON 입력` 메뉴에서 새 단지 JSON을 입력합니다.
2. 검증 후 관리자 저장 키를 입력하고 `새 단지 저장`을 누릅니다.
3. 신규 JSON 파일이 생성되고, 재배포 후 단지 목록과 비교 화면에 표시됩니다.

단지를 삭제할 때:

1. `단지` 탭의 해당 단지 카드 또는 단지 상세 화면에서 `단지 삭제`를 누릅니다.
2. 삭제 확인을 위해 단지명을 정확히 입력하고 `ADMIN_SAVE_KEY` 값을 입력합니다.
3. `단지 및 매물 삭제`를 누르면 해당 단지 JSON 파일과 그 안의 모든 매물이 GitHub 삭제 커밋으로 제거됩니다.
4. Cloudflare Pages 재배포가 완료되면 단지 목록, 비교 그룹, 대시보드에서 사라집니다.
