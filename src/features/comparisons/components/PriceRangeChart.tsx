...
export function PriceRangeChart({ summaries, showArea = false }: { summaries: ListingAreaSummary[]; showArea?: boolean }) {
  const data = summaries.map((summary) => ({
    name: `${summary.complex_name.length > 8 ? `${summary.complex_name.slice(0, 8)}…` : summary.complex_name}${
  ...
  return (
    <Card>
      <h2 className="text-base font-semibold">{showArea ? '단지·평형별 가격 범위' : '단지별 가격 범위'}</h2>
      <p className="mt-1 text-xs text-slate-400">최저가부터 최고가까지의 현재 매매 호가 범위입니다.</p>
      <div className={`mt-5 ${showArea ? 'h-[420px]' : 'h-64'}`}>
        <ResponsiveContainer width="100%" height="100%">
          <BarChart data={data} layout="vertical" margin={{ left: 4, right: 20 }}>
            ...
            <YAxis dataKey="name" type="category" width={showArea ? 124 : 90} fontSize={11} stroke="#94a3b8" />
            <Tooltip
              cursor={{ fill: '#f8fafc' }}
              formatter={(_value, _key, item) => {
                const payload = item.payload as (typeof data)[number];
                return [`${payload.label}: ${formatPrice(payload.minimum)} ~ ${formatPrice(payload.maximum)} (평균 ${formatPrice(payload.average)})`, '호가'];
              }}
            />
